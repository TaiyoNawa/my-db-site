...
## フェーズ8: 追加要望その2（2026-07-09 追記）

- [x] 通話ステータスに「応答なし」を追加（型・store・Composerメニュー・編集Select）
- [x] 不在着信/キャンセル/応答なしのアイコンを電話アイコン1種に統一し、横並び（アイコン+ラベル）表示に変更
  - 通話成立時（アイコン下に通話時間）は既存UIのまま変更なし
- [x] MessageComposer を2行レイアウトに再構成し、入力欄のPlaceholderが折り返して見切れる問題を修正
  - 1行目: 相手/自分トグル + 入力欄 + 送信ボタン
  - 2行目: ＋メニュー + 画像添付 + （グループ時）送信メンバー選択
- [x] テスト追加（通話ラベル4パターン・store noAnswer）+ test/lint/build 全パス

## フェーズ9: 追加要望その3（2026-07-09 追記）

- [x] 入力バーの「＋」メニューを画像添付ボタンの左に移動（1行目内）
- [x] 通話UIを全ステータス共通の横並び（丸い濃色バッジ付きアイコン + ラベル）に統一
  - 通話成立時も含めて統一。バッジは blackAlpha オーバーレイでテーマ色を問わず「地の色より少し濃い丸」を実現
  - 通話メッセージのみ縦paddingを py=3 に拡大（他は py=2 のまま）
- [x] ヘッダーに検索アイコンを追加（電話アイコンの左）
- [x] test/lint/build 全パス（既存49テストは表示テキストが変わらず無修正で通過）

## フェーズ10: 追加要望その4（2026-07-09 追記）

- [x] 通話成立時のUIを刷新: 「通話メッセージ（既定文言、編集可） + 下に小さく通話時間」の2段構成に変更
  - DEFAULT_CALL_COMPLETED_TEXT('音声通話が終了しました')を追加、addCallMessageで自動セット
  - 編集ポップオーバーに通話メッセージ用のTextareaを追加、Select変更時に空文言なら自動補完
- [x] 不在着信/キャンセル/応答なしはフォント15px・アイコン間隔10pxに拡大し余裕を持たせた
- [x] 通話バブル全体の左右paddingも4に拡大（縦は3のまま）
- [x] test/lint/build 全パス（store 1件テキスト検証追加）
