...
describe('AnimeList.tsxのテスト', () => {
  it('キーワードが空のときは何も表示されない', () => {
    renderWithChakra(<AnimeList keyword="" />);
  ...
  it('検索結果の見出しが表示される', async () => {
    ...
    renderWithChakra(<AnimeList keyword="test" />);
  ...
  it('該当するアニメが見つからないときの表示', async () => {
    ...
    renderWithChakra(<AnimeList keyword="noresult" />);

    await waitFor(() =>
      expect(
        screen.getByText('該当するアニメが見つかりませんでした。')
      ).toBeInTheDocument()
    );
  });
  ...
  it('さらに読み込むボタンが表示される', async () => {
    ...
    renderWithChakra(<AnimeList keyword="test" onReset={mockHandleLoadMore} />);
    await waitFor(() =>
      expect(screen.getByText('"test"の検索結果')).toBeInTheDocument()
    );
    await waitFor(() =>
