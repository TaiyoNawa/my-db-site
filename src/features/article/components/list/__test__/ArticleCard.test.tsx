...
describe("Article.tsxのテスト", () => {
  ...
  it("titleが100文字以内で表示される", () => {
    render(<ArticleCard {...testProps2} />);
    const title = screen.getByText(testProps2.title.slice(0, 100) + "...");
    expect(title).toBeInTheDocument();
    expect(title.textContent?.length).toBeLessThanOrEqual(103);
  });
  it("descriptionが100文字以内で表示される", () => {
    render(<ArticleCard {...testProps2} />);
    const description = screen.getByText(
      testProps2.description.slice(0, 100) + "..."
    );
