'use client';
// pages/gallery/tool/poll/[pollUid]/index.tsx
// /gallery/poll/[pollUid] から /gallery/tool/poll/[pollUid] に移動済み
import {
  Box,
  Button,
  Checkbox,
...
const PollPage = () => {
  ...
  const swrResponse: SWRResponse<PollData, Error> = useSWR<
    PollData,
    Error,
    string | null
  >(
  ...
  useEffect(() => {
    ...
    if (isDeadlinePassed) {
      const resultsUrl = `/gallery/tool/poll/${poll.pollUid}/results`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`/api/poll/${String(pollUid)}/submit`, {
        ...
        body: JSON.stringify({
          voterId,
          answers: Object.keys(answers).map((questionUid) => ({
            questionUid,
            answer: answers[questionUid],
          })),
        }),
      });
      ...
      if (!response.ok) {
        ...
        if (response.status === 409) {
          ...
          void router.push(`/gallery/tool/poll/${String(pollUid)}/results`);
    ...
    return (
      <>
        <GalleryMeta
          ...
          ogUrl={`/gallery/tool/poll/${poll?.pollUid || ''}`}
        />
